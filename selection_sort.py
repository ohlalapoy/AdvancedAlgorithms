...
'''
ดีกว่า Bogo ขึ้นมาหน่อยเพราะจะวนเทียบ Data จาก minimum
ไปเทียบกับทุกตัวที่อยู่ใน list แล้วเอ่าไป append กับ list ใหม่ 
วนจนครบไม่เหลือของเก่าใน List เดิมตอนแรก 
'''
# execute program with command python selection_sort.py numbers/5.txt
...
def selection_sort(values):
  ...
  sorted_list = [] # 1) list เปล่าไว้เก็บผลลัพธ์ที่เรียงแล้ว
  print("%-25s %-25s" % (values, sorted_list)) # 2) print สถานะก่อนเริ่ม
  ...
  for i in range(0, len(values)):
    ...
    index_to_move = index_of_min(values)  # 3) หา index ของค่าน้อยที่สุดใน values
    ...
    # sorted list.
    sorted_list.append(values.pop(index_to_move)) # 4) ดึงค่านั้นออกจาก values แล้วใส่เข้า sorted_list\
    print("%-25s %-25s" % (values, sorted_list))   # 5) print สถานะหลังแต่ละรอบ ให้เห็นความคืบหน้า
